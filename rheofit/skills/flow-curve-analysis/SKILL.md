...
| `carreau_carreau` | $\sigma = \eta_{0,1}\dot\gamma[1+(\lambda_1\dot\gamma)^2]^{-1/4} + \eta_{0,2}\dot\gamma[1+(\lambda_2\dot\gamma)^2]^{-1/2}$ | Two distinct relaxation processes with **fixed** exponents: mode 1 ($n_1=1/2$) for polymer–surfactant interaction or branched WLM, mode 2 ($n_2=0$) for worm-like micelles. No yield stress. | $\eta_{0,1}, \lambda_1, \eta_{0,2}, \lambda_2$ per component — exponents are not fitted |
...
$\lambda_i$ are free; each mode costs two well-identified parameters instead of three. In the
two-mode models index 1 is always the $n=1/2$ mode and index 2 the $n=0$ WLM mode. `tc_carreau`
...
| `tccc`             | $\sigma = \sigma_y + \sigma_y\sqrt{\dot\gamma/\dot\gamma_c} + \eta_{0,1}\dot\gamma[1+(\lambda_1\dot\gamma)^2]^{-1/4} + \eta_{0,2}\dot\gamma[1+(\lambda_2\dot\gamma)^2]^{-1/2}$ | Yield stress plus both fixed-exponent modes: $n_1=1/2$ (polymer–surfactant interaction or branched WLM) and $n_2=0$ (worm-like micelles). Richest model — needs well-sampled data. | $\sigma_y$, $\dot\gamma_c$, $\eta_{0,1}, \lambda_1, \eta_{0,2}, \lambda_2$              |
